...
from infoduel_maddpg.utils.networks import MLPNetwork
from infoduel_maddpg.utils.misc import (
    soft_update,
    average_gradients,
    onehot_from_logits,
    gumbel_softmax,
)
from infoduel_maddpg.utils.agents import DDPGAgent
...
class MADDPG(object):
    ...
    def __init__(
        ...
        self.agents = [
            DDPGAgent(
                lr=lr, discrete_action=discrete_action, hidden_dim=hidden_dim, **params
            )
    ...
    def update(self, sample, agent_i, parallel=False, logger=None):
        ...
        if self.alg_types[agent_i] == "MADDPG":
            if self.discrete_action:  # one-hot encode action
                ...
                all_trgt_acs = [
                    pi(nobs) for pi, nobs in zip(self.target_policies, next_obs)
                ]
            ...
            if self.discrete_action:
                ...
                trgt_vf_in = torch.cat(
                    (next_obs[agent_i], curr_agent.target_policy(next_obs[agent_i])),
                    dim=1,
                )
        target_value = rews[agent_i].view(
            -1, 1
    ...
    def init_from_env(
        ...
        for acsp, obsp, algtype in zip(action_space, observation_space, alg_types):
            ...
            if algtype == "MADDPG":
                num_in_critic = 0
                for oobsp in observation_space:
                    num_in_critic += oobsp.shape[0]
                for oacsp in action_space:
